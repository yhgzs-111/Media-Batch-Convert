...
def convert_media_files():
    ...
    response = messagebox.askquestion("Tip", "Are you sure you want to convert in bulk?")
    if response != 'yes':
        return
...
def convert(source_folder, destination_folder, output_format, info_window, progress_var):
    try:
        ...
        media_files = [f for f in os.listdir(source_folder) if os.path.isfile(os.path.join(source_folder, f))]  # Get all files in the target folder
        if not any(check_media_file(os.path.join(source_folder, file)) for file in media_files):
            raise ValueError("No audio or video files in the target folder")
        ...
        info_window.destroy()
        root.deiconify()
        messagebox.showinfo("Tip", "Batch conversion completed!")
...
root = tk.Tk()
root.title("Media Batch Converter")
