...
def convert_media_files():
    ...
    if not (source_folder and destination_folder and output_format):
        messagebox.showerror("錯誤", "請選擇目標文件夾、輸出文件夾和輸出格式")
        return

    if output_format not in audio_formats + video_formats:
        messagebox.showerror("錯誤", "不支援的輸出格式")
        return

    response = messagebox.askquestion("提示", "確定要批次轉換嗎？")
    if response != 'yes':
        return
    ...
    label = tk.Label(info_window, text="正在轉換，請稍候……", font=("Arial", 16))
    label.pack(pady=5)
...
def convert(source_folder, destination_folder, output_format, info_window, progress_var):
    try:
        ...
        media_files = [f for f in os.listdir(source_folder) if os.path.isfile(os.path.join(source_folder, f))]  # 獲取目標文件夾中的所有文件
        if not any(check_media_file(os.path.join(source_folder, file)) for file in media_files):
            raise ValueError("目標文件夾中沒有音頻或影片文件")
        ...
        info_window.destroy()
        root.deiconify()
        messagebox.showinfo("提示", "批次轉換完成！")
    except Exception as e:
        info_window.destroy()
        root.deiconify()
        messagebox.showerror("錯誤", f"轉換出錯: {str(e)}")
    finally:
...
destination_button = tk.Button(root, text="選擇輸出文件夾", command=select_destination_folder)
destination_button.grid(row=1, column=2, padx=10, pady=10)

output_format_label = tk.Label(root, text="輸出格式:")
...
convert_button = tk.Button(root, text="開始轉換", command=convert_media_files)
