...
def convert_media_files():
    ...
    if not (source_folder and destination_folder and output_format):
        messagebox.showerror("错误", "请选择目标文件夹、输出文件夹和输出格式")
        return
    ...
    response = messagebox.askquestion("提示", "确定要批量转换吗？")
    if response != 'yes':
        return
...
def convert(source_folder, destination_folder, output_format, info_window, progress_var):
    try:
        ...
        media_files = [f for f in os.listdir(source_folder) if os.path.isfile(os.path.join(source_folder, f))]  # 获取目标文件夹中的所有文件
        if not any(check_media_file(os.path.join(source_folder, file)) for file in media_files):
            raise ValueError("目标文件夹中没有音视频文件")
...
output_format_label = tk.Label(root, text="输出格式:")
...
convert_button = tk.Button(root, text="开始转换", command=convert_media_files)
